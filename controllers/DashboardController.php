    ...
    public function beforeAction($action)
    {
        ...
        if (file_exists(Yii::getAlias('@app/install.php'))) {
            Yii::$app->getSession()->setFlash(
                'danger',
                Yii::t('app', "For security reasons you must remove the file 'install.php' from your application directory.")
            );
        }

        if (is_writable(Yii::getAlias('@app/config/db.php'))) {
            Yii::$app->getSession()->setFlash(
                'danger',
                Yii::t('app', "Your 'config/db.php' file is writable for the web server. For security reasons you must change it (chmod 0644).")
            );
        }
