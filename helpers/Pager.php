...
class Pager
{
    ...
    protected function loadNavigationLinks($data)
    {
        ...
        preg_match_all('#data-button-previous="(.*?)" data-button-next="(.*?)"#U', $data, $matches, PREG_SET_ORDER);
            ...
            foreach ($this->pages as $currentPageIndex => $currentPage) {
                if ($currentPageIndex === $firstPageIndex) { // First page
                    ...
                    $replacement = array(
                        ...
                        $this->navigationLinks[$currentPageIndex-1]['previous'] ." ". '<input type="submit"',
                        $this->navigationLinks[$currentPageIndex-1]['previous'] ." ". '<input type="image"',
                    );
                    $currentPage = str_replace($pattern, $replacement, $currentPage);
