...
class Form extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['message'], 'string'],
            [['status', 'schedule', 'schedule_start_date', 'schedule_end_date',
                'total_limit', 'total_limit_number', 'ip_limit', 'ip_limit_number',
                'save', 'resume', 'autocomplete', 'analytics', 'honeypot', 'recaptcha',
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if ($insert) {
            ...
            $filesDirectory = $this::FILES_DIRECTORY . DIRECTORY_SEPARATOR . $this->id;
        ...
        if ($this->recaptcha === $this::RECAPTCHA_ACTIVE && !Yii::$app->session['reCaptcha']) {
            ...
            if (!$recaptchaValidator->validate($post[$recaptchaValidator::CAPTCHA_RESPONSE_FIELD], $message)) {
                ...
                $response->data = array(
                    ...
                    'success' => false,
                    'id' => 0,
                    'message' => Yii::t("app", "Error of validation."),
                    'errors' => [$errors],
                );
                $response->send();
